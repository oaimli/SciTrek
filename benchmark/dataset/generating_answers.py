...
if __name__ == "__main__":
    # regenerate answers for all questions produced by Qwen2.5 before human validation and model training/testing
    # because the updates in papers_final.json affect answers for some questions
    with open("../article/papers_final.json") as f:
        papers_all = json.load(f)

    for prefix in ["single", "64k", "128k", "512k", "1024k"]:
        if prefix in ["512k", "1024k"]:
            qwen_samples = []
            with jsonlines.open(f"samples/qwen25/{prefix}_samples_target.jsonl") as reader:
                for line in reader:
                    qwen_samples.append(line)
            other_samples = []
            with jsonlines.open(f"samples/other/{prefix}_samples_target.jsonl") as reader:
                for line in reader:
                    other_samples.append(line)
            assert len(qwen_samples) == len(other_samples)

            final_samples = []
            invalid_samples = []
            for sample_x_index, (sample_x, sample_y) in enumerate(zip(qwen_samples, other_samples)):
                assert sample_x["articles"] == sample_y["articles"]
                assert sample_x["template"] == sample_y["template"]
                assert sample_x["label"] == sample_y["label"]
                label = sample_y["label"]
                if label == "test":
                    sample_final = sample_x
                else:
                    sample_final = sample_y
                if sample_final["question"] != "":
                    cluster_papers = []
                    for paper_id in sample_final["articles"]:
                    ...
                    cur = con.cursor()
                    original_result = []
                    original_execution = cur.execute(sample_final["sql"]).fetchall()
                    if original_execution != None:
                        for row in original_execution:
                            x = [str(y) for y in row]
                            if len(x) > 0:
                                original_result.append(", ".join(x))
                    sample_final["answer"] = ", ".join(original_result) if len(original_result) > 0 else ""

                    if sample_final["answer"] == "" or sample_final["answer"] == "None":
                        sample_final["answer"] = "NULL"
                    qwen_samples[sample_x_index] = sample_final
                else:
                    invalid_samples.append(sample_x)
            print("invalid samples", len(invalid_samples))

            save_file = f"samples/final/{prefix}_samples_target.jsonl"
            with jsonlines.open(save_file, "w") as writer:
                writer.write_all(qwen_samples)
                print(prefix, len(qwen_samples))
        else:
            qwen_samples = []
            with jsonlines.open(f"samples/qwen25/{prefix}_samples_target.jsonl") as reader:
                for line in reader:
