...
if __name__ == "__main__":
    ...
    for prefix in ["single", "64k", "128k", "512k", "1024k"]:
        save_file = f"../samples/gpt41/{prefix}_samples_target.jsonl"
        gpt4_samples = []
        with jsonlines.open(save_file) as reader:
            for line in reader:
                gpt4_samples.append(line)

        for sample_x_index, sample_x in enumerate(gpt4_samples):
            if sample_x["question"] != "":
                ...
                gpt4_samples[sample_x_index] = sample_x

        with jsonlines.open(save_file, "w") as writer:
            writer.write_all(gpt4_samples)