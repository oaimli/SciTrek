...
if __name__ == "__main__":
    ...
    gpt4_samples_single = []
    with jsonlines.open("../samples/llama33/single_samples_target.jsonl") as reader:
        for line in reader:
            gpt4_samples_single.append(line)
    gpt4_samples_64k = []
    with jsonlines.open("../samples/llama33/64k_samples_target.jsonl") as reader:
        for line in reader:
            gpt4_samples_64k.append(line)
    gpt4_samples_128k = []
    with jsonlines.open("../samples/llama33/128k_samples_target.jsonl") as reader:
        for line in reader:
            gpt4_samples_128k.append(line)
    gpt4_samples_512k = []
    with jsonlines.open("../samples/llama33/512k_samples_target.jsonl") as reader:
        for line in reader:
            gpt4_samples_512k.append(line)
    gpt4_samples_1024k = []
    with jsonlines.open("../samples/llama33/1024k_samples_target.jsonl") as reader:
        for line in reader:
            gpt4_samples_1024k.append(line)

    for prefix in ["64k", "128k", "512k", "1024k", "single"]:
        if prefix == "64k":
            for sample_x_index, sample_x in enumerate(gpt4_samples_64k):
                ...
                if len(questions_done) > 0:
                    ...
                    gpt4_samples_64k[sample_x_index] = sample_x

            with jsonlines.open("../samples/llama33/64k_samples_target.jsonl", "w") as writer:
                writer.write_all(gpt4_samples_64k)
        if prefix == "128k":
            for sample_x_index, sample_x in enumerate(gpt4_samples_128k):
                sample_x_sql = sample_x["sql"]
                questions_done = set([])
                for sample_y in gpt4_samples_single + gpt4_samples_64k + gpt4_samples_128k + gpt4_samples_512k + gpt4_samples_1024k:
                    sample_y_sql = sample_y["sql"]
                ...
                if len(questions_done) > 0:
                    ...
                    gpt4_samples_128k[sample_x_index] = sample_x

            with jsonlines.open("../samples/llama33/128k_samples_target.jsonl", "w") as writer:
                writer.write_all(gpt4_samples_128k)
        if prefix == "512k":
            ...
            with jsonlines.open("../samples/llama33/512k_samples_target.jsonl", "w") as writer:
        ...
        if prefix == "1024k":
            ...
            with jsonlines.open("../samples/llama33/1024k_samples_target.jsonl", "w") as writer:
                writer.write_all(gpt4_samples_1024k)
        if prefix == "single":
            for sample_x_index, sample_x in enumerate(gpt4_samples_single):
                sample_x_sql = sample_x["sql"]
                questions_done = set([])
                for sample_y in gpt4_samples_single + gpt4_samples_64k + gpt4_samples_128k + gpt4_samples_512k + gpt4_samples_1024k:
                    sample_y_sql = sample_y["sql"]
                ...
                if len(questions_done) > 0:
                    ...
                    gpt4_samples_single[sample_x_index] = sample_x

            with jsonlines.open("../samples/llama33/single_samples_target.jsonl", "w") as writer:
                writer.write_all(gpt4_samples_single)
