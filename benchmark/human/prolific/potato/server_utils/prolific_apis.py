...
class ProlificBase(object):
    ...
    def get_submissions_from_study(self, study_id = None):
        ...
        api_endpoint = "https://api.prolific.com/api/v1/studies/{}/submissions/"
...
class ProlificStudy(ProlificBase):
    ...
    # transition submissions from AWAITING REVIEW
    def reject_submissions(self, output_annotation_dir, rejection_accumulation):
        print('rejection_accumulation', rejection_accumulation)
        print('output_annotation_dir', output_annotation_dir)
        submission_data = self.get_submissions_from_study()
        ...
        if submission_data != None:
            for submission in submission_data:
                ...
                if status == "AWAITING REVIEW":
                    print(submission_id, status, participant_id)
                    user_annotation_tmp = os.path.join(output_annotation_dir, participant_id, "annotated_instances.jsonl")
                    # print(user_annotation_tmp)
                    if os.path.exists(user_annotation_tmp):
                        failed_test = False
                        test_id = ""
                        original_answer_record = ""
                        annotated_answer_record = ""
                        question_record = ""
                        with jsonlines.open(user_annotation_tmp) as reader:
                            for line in reader:
                                if line["id"].endswith("_testing"):
                                    test_id = line["id"]
                                    annotated_answer = line["label_annotations"]["answer"]["text_box"].strip()
                                    annotated_answer = " ".join(annotated_answer.split())
                                    annotated_answer = ", ".join([element.strip() for element in annotated_answer.split(",")])
                                    original_answer = line["meta"]["answer"].strip()
                                    if annotated_answer != original_answer:
                                        print("original answer", original_answer)
                                        print("annotated answer", annotated_answer)
                                        original_answer_record = original_answer
                                        annotated_answer_record = line["label_annotations"]["answer"]["text_box"]
                                        question_record = line["meta"]["question"].strip()
                                        failed_test = True
                                        break
                        print(submission_id, failed_test, test_id)
                        rejected_count = rejection_accumulation.get(test_id, 0)
                        if failed_test and rejected_count <= 10:
                            print(f'rejecting submission {submission_id} from user {participant_id} because of {test_id}')
                            api_endpoint = 'https://api.prolific.com/api/v1/submissions/{}/transition/'
                            url = api_endpoint.format(submission_id)
                            data = {
                                "action": "REJECT",
                                "message": f"Sorry that we have to reject your response to the study. It is because that your answer did not pass the quality control test that we put in the study (we already have the correct answer for one of the samples). For one sample where the question is {question_record}, your response was {annotated_answer_record} while the correct answer should be {original_answer_record}. We've already deleted your data. If you still have questions on this, please message us via Prolific and we could consider to send you the request to return the study.",
                            ...
                            if "error" in data.keys():
                                print(f"Error, failed to reject the submission {submission_id}.")
                            else:
                                if "status" in data.keys():
                                    if data["status"] == "REJECTED":
                                        rejected_tests.append(test_id)
                                        print("Rejected submission for user %s" % participant_id)
                    else:
                        print(f"{user_annotation_tmp} does not exist.")
    ...
    def get_dropped_users(self):
        # print(self.user_status_dict.keys())
