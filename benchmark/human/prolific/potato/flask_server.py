...
# accumulated rejection of different test samples for quality control
...
class UserAnnotationState:
    ...
    def __init__(self, assigned_user_data):
        ...
        # therefore, we construct a separate dictionary to save all the
...
def home():
    ...
    if "login" in config:
        ...
        print("You have to specify a login method in the config.")
        quit()
...
def assign_instances_to_user(username):
    ...
    assigned_ids_tmp = []
    for key_tmp in user_state.get_assigned_data().keys():
        if not key_tmp.endswith(".html"):
            assigned_ids_tmp.append(key_tmp)
    print("assigned ids", assigned_ids_tmp)
...
def remove_instances_from_users(user_set):
    ...
    print("the task assignment", task_assignment)
    if len(user_set) == 0:
        print('No users need to be dropped at this moment')
        return None
    else:
        print("Dropped users:", user_set)
    ...
    for inst_id in task_assignment['assigned']:
        ...
        for u in task_assignment['assigned'][inst_id]:
            if u in user_set:
                if not inst_id.endswith("_testing"):
    ...
    if len(user_set) > 0:
        print("updated task assignment", task_assignment)
...
def update_prolific_study_status():
    ...
    global user_to_annotation_state
    global rejection_accumulation

    print('reject_submissions is called')
    reject_tests = prolific_study.reject_submissions(config["output_annotation_dir"], rejection_accumulation)
    for reject_test in reject_tests:
        rejection_accumulation[reject_test] = rejection_accumulation.get(reject_test, 0) + 1
    print("rejection_accumulation = %s" % rejection_accumulation)
...
def save_user_state(username, save_order=False):
    ...
    with open(annotated_instances_fname, "wt") as outf:
        for inst_id, data in user_state.get_all_annotations().items():
            ...
            output = {
                "id": inst_id,
                "meta": instance_id_to_data[inst_id]["meta"],
    ...
    if "list_as_text" in config and config["list_as_text"]:
        ...
        # unfolding dict into different sections
            ...
            def render_table(data, title):
                table = (
                    f'<p style="text-align:center; margin-bottom: 10px;"><strong>{title}</strong></p>'
                ...
                for i, row in enumerate(data):
                    ...
                    for cell in row:
                        ...
                        cell_style = (
                            ...
                            "background-color: #A9A9A9;" if tag == "th" else
                            "padding: 8px; border: 1px solid #ddd; text-align: center; background-color: #F5F5F5;"
                        )
                        table += f'<{tag} style="{cell_style}">{cell}</{tag}>'
                    table += "</tr>"
                table += "</table>"

                return table

            articles = text["table_articles"]
            article_author = text["table_article_author"]
            citing_cited = text["table_citing_cited"]

            html_parts = [
                render_table(articles, "Table 1: Articles with Meta Information"),
                render_table(article_author, "Table 2: Authors for The Articles"),
                render_table(citing_cited, "Table 3: Citation Relations Among The Articles")
                ]

            question = f'<h5 style="color: darkred; user-select: none;"><strong>Question: {text["question"]}</strong></h5>'
            text = (
                    '<div style="text-align:center; margin: 20px auto; font-family: Arial, sans-serif;">'
                    + "<br><br>".join(html_parts) +
                    f'<br><br>{question}</div>'
            )
        else:
            text = text
    return text
...
def annotate_page(username=None, action=None):
    ...
    if instance_id in config.get("non_annotation_pages", []):
        ...
        next_hidden = True
        if instance_id.endswith("-intro.html"):
            start_hidden = False
        else:
            start_hidden = True
        html_file = instance_id
    ...
    rendered_html = render_template(
        ...
        custom_js=custom_js,
        next_hidden=('next_hidden' if next_hidden else ''),
        start_hidden=('start_hidden' if start_hidden else ''),
        **kwargs
    )
...
def run_server(args):
    ...
    if config.get('prolific') and config['prolific']['config_file_path']:
        ...
        for k,v in study_basic_info.items():
            print("%s: %s"%(k, v))
        print("output directory: %s"%config["output_annotation_dir"])
