...
if __name__ == "__main__":
    ...
    with open(os.path.join("../benchmark/article/", "papers_final.json")) as f:
    ...
    for generation_result in os.listdir(result_folder):
        if generation_result.endswith(".jsonl") and "test_full" in generation_result:
            tmp = []
    ...
    for model in models:
        ...
        for test_focus in test_focuses:
            ...
            generation_batches = []
            for generation_result in os.listdir(result_folder):
                if generation_result.endswith(".jsonl") and "test_full" in generation_result and model in generation_result and "128k" in generation_result:
                    with jsonlines.open(os.path.join(result_folder, f"{generation_result}")) as reader:
                        for line in reader:
        ...
        for domain in domains:
            ...
            for generation_result in os.listdir(result_folder):
                if generation_result.endswith(
                        ".jsonl") and "test_full" in generation_result and model in generation_result and "128k" in generation_result:
                    with jsonlines.open(os.path.join(result_folder, f"{generation_result}")) as reader:
                        for line in reader:
    ...
    # output json
    with open("fine_grained_filtering_128k_em_full.json", mode="w") as f:
        json.dump(em_results, f)
    with open("fine_grained_filtering_128k_f1_full.json", mode="w") as f:
        json.dump(f1_results, f)
    with open("fine_grained_filtering_128k_rouge_l_full.json", mode="w") as f:
        json.dump(rouge_l_results, f)

    # output excel
    df = pd.DataFrame.from_dict(all_results, orient="columns")
    df.index.name = "row"
    df.to_excel("fine_grained_filtering_128k_full.xlsx", sheet_name="more")
